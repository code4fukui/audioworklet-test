# オーディオワークレットのテスト

1〜2文で説明。WebAudioAPI の AudioWorkletの機能テスト集。

## デモ
- [シンプルなビープ音](https://code4fukui.github.io/audioworklet-test/beep.html)
- [シンプルな複数のビープ音](https://code4fukui.github.io/audioworklet-test/beeps.html)
- [シンプルなノイズ音](https://code4fukui.github.io/audioworklet-test/noise.html)
- [PAPU音源テスト](https://code4fukui.github.io/audioworklet-test/papu.html)
- [IchigoJam MML](https://code4fukui.github.io/audioworklet-test/mml.html)
- [サイン波](https://code4fukui.github.io/audioworklet-test/sine.html)
- [風の音](https://code4fukui.github.io/audioworklet-test/wind.html)
- [サンプリング音源 - クラッカーの音](https://code4fukui.github.io/audioworklet-test/samples.html)

## 機能
- AudioWorkletProcessorによるカスタムオーディオノードの作成
- 波形合成によるビープ音の生成
- サンプリング音源の再生
- IchigoJam風MMLの再生
- PAMソーンドチップ(PAPU)の音源再生

## 必要環境
- 最新のWebブラウザ
- AudioWorkletをサポートしていること

## 使い方
リポジトリをクローンし、各HTMLファイルを開くことで各デモを試すことができます。

## ライセンス
MITライセンス